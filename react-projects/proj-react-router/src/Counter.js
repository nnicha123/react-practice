            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div className="wrapper">
                    <div className="Counter">
                        ...
                        <span>
                            <button className="buttonCount" onClick={this.reset}>Reset</button>
                            <button className="buttonCount" onClick={this.add}>+</button>
                            <button className="buttonCount" onClick={this.subtract}>-</button>
                        </span>
                ...
                <div>
                    ...
                    <p>Select Operator to perform
                        <button className="buttonCount" onClick={() => this.setState({ operator: '+' })}>+</button>
                        <button className="buttonCount" onClick={() => this.setState({ operator: '-' })}>-</button>
                        <button className="buttonCount" onClick={() => this.setState({ operator: 'x' })}>x</button>
                        <button className="buttonCount" onClick={() => this.setState({ operator: '/' })}>/</button>
                        {(this.state.operator === '-') && <span style={{ color: 'tomato', fontWeight: 'bolder' }}>Subtract</span>}
                        {(this.state.operator === '+') && <span style={{ color: 'tomato', fontWeight: 'bolder' }}>Add</span>}
                        {(this.state.operator === 'x') && <span style={{ color: 'tomato', fontWeight: 'bolder' }}>Multiply</span>}
                        {(this.state.operator === '/') && <span style={{ color: 'tomato', fontWeight: 'bolder' }}>Divide</span>}
                    </p>
                    <p style={{ textAlign: "center" }}>Press
                    <button className="counterValue" style={{ margin: '10px', width: 'max-content', backgroundColor: "tomato", color: 'white' }}onClick={this.compute}>=</button> to complete</p>
                    <span style={{ display: 'flex', marginBottom: '20px', width: '100%', justifyContent: 'center' }}>
